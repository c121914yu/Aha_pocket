...
<template>
	<view class="project-head">
		<view class="sort-filter" :class="topRadius ? 'top-radius' : ''">
			<text :class="index === sortActive ? 'active' : ''" v-for="(sort, index) in sortList" :key="sort" @click="checkSort(sort, index)">{{ sort.text }}</text>
			<text
				:style="{
...
export default {
	...
	data() {
		return {
			sortList: [{ text: '综合', val: 'read' }, { text: '收藏量', val: 'collect' }, { text: '最新', val: 'time' }, { text: '获奖等级', val: 'awardLevel' }],
			sortActive: 0,
